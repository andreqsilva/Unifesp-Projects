...
public class Car implements Runnable{
	...
	if (cartype==REDCAR) {
        if (inFront.getX() - xpos > 100) {
            ...
            if (xpos >= bridgeXLeft & xposOld < bridgeXLeft) controller.enterBridge();
		    ...
		    else if (xpos >= bridgeXRight &&  xposOld < bridgeXRight) controller.leaveBridge();
        }
	} else {
        if (xpos-inFront.getX() > 100) {
            xpos -= 4;
            //if (xpos <= bridgeXRight && xposOld > bridgeXRight) controller.enterRight();
            if (xpos <= bridgeXRight && xposOld > bridgeXRight) controller.enterBridge();
		    ...
		    else if (xpos <= bridgeXLeft && xposOld > bridgeXLeft) controller.leaveBridge();
        }
	}
    }
