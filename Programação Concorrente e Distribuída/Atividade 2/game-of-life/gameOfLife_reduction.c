...
int runGenerations_reduction() {
  ...
  for (generation = 0; generation < G; generation++) {
    ...
    #pragma omp parallel shared(generation) private(i,j,neighbors) reduction(+:livingGenerations) num_threads(T)
    {
    #pragma omp for
      for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
...
int main(int argc, char **argv) {
  ...
  setInitGrid(grid);
  //imprime(grid);
  livingGenerations = runGenerations_reduction();
